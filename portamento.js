				...
				if (y >= (topScrollBoundary)) { // if we're at or past the upper scrolling boundary
					if((panel.innerHeight() - wrapper.viewportOffset().top) - wrapperPaddingFix + gap >= wrapper.height()) { // if we're at or past the bottom scrolling boundary
						if(panel.hasClass('fixed') || thisWindow.height() >= panel.outerHeight()) { // check that there's work to do
							...
							// Calculate the height of any elements appearing BEFORE the panel.
							var previousElementsHeight = 0;
							var previousElements = panel.parent().prevAll();
							previousElements.each(function() {
								previousElementsHeight += $(this).outerHeight();
							});

							panel.css('top', (wrapper.innerHeight() - panel.innerHeight() - previousElementsHeight) + 'px');
						}
